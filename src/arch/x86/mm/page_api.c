...
int x86_pte_init()
{
        ...
        memset((void*) &pte_cnt, 0, sizeof(pte_cnt));
#ifdef SLAB
...
int x86_pte_load_range(struct sys_mmu_range* range)
{
        ...
        if (meta == NULL) {
                ...
                if (meta == NULL)
                        panic(
                                        "Unable to allocate paging meta data for segment");
        ...
        skip2: return -E_SUCCESS;
        mapped: printf("PDE: %X\tPTE: %X\n", (int) idx, (int) i);

...
int vm_range_update()
{
        panic("Using unimplemented function, vm_range_update");
        return 0;
}
...
void* vm_get_kernel_heap_pages(size_t size)
{
        ...
        panic("Using unimplemented function, vm_kernel_get_heap_pages");
        return NULL;
}
...
mm_slab_alloc(struct mm_slab* slab, int flags)
{
        ...
        if ((addr_t) slab->obj_ptr % slab->cache->alignment != 0) {
                log(stderr, "Obj ptr alignment is off by: %X\n",
                                (size_t) slab->obj_ptr % slab->cache->alignment);
        }
        if (tmp % slab->cache->alignment != 0) {
                log(stderr, "Allocated object off alignment by: %X\n",
        ...
        if (cache->slabs_partial == NULL) {
                ...
                if (tmp == NULL) {
                        ...
                        struct mm_slab* slab = vm_get_kernel_heap_pages(
                                        no_pages);
                        if (slab == NULL) {
                                log(stdwarn, "Unable to do memory allocation at"
                                                " this time!\n");
                                goto err;
                        }
